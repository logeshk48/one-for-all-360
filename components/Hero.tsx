      ...
      <div className="shell">
        <div className="grid items-center gap-y-10 lg:grid-cols-12 lg:gap-x-10">
          <div className="lg:col-span-6 xl:col-span-5">
            ...
            <Reveal delay={80}>
              <h1 className="display mt-6 text-[3.5rem] sm:text-[4.75rem] lg:text-[5.25rem] xl:text-[6.25rem]">Everything.<br />One Place<span className="text-accent">.</span></h1>
            </Reveal>

            <Reveal delay={150}>
              <p className="mt-8 max-w-md text-base leading-relaxed text-ink/60 md:text-lg">{hero.support}</p>
            </Reveal>

            <Reveal delay={220}>
              <div className="mt-10 flex flex-col items-start gap-2 sm:flex-row sm:items-center sm:gap-10">
                <ActionLink href={hero.primaryCta.href} label={hero.primaryCta.label} />
                <ActionLink href={hero.secondaryCta.href} label={hero.secondaryCta.label} muted />
              </div>
            </Reveal>
          </div>

          <div className="lg:col-span-6 lg:col-start-7 xl:col-span-6 xl:col-start-7">
            <Reveal delay={180} className="mx-auto max-w-[34rem] lg:max-w-none">
              <EcosystemVisual />
            </Reveal>
          </div>
        </div>
          ...
          <div className="mt-8 border-t border-mist py-6 md:mt-14">
            <ul className="flex flex-wrap items-center gap-x-6 gap-y-2">
              {industries.map((industry) => (
                <li key={industry.short} className="eyebrow text-ink/35">{industry.short}</li>
              ))}
